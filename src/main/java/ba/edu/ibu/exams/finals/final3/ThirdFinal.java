...
public class ThirdFinal {
    ...
    public ThirdFinal(String ipAddress, int portNumber) {
        this.populations = loadPopulations(ipAddress, portNumber);
    }

    public static List<String> readFileFromSocket(String address, int port) {
        try {
    ...
    private static List<Population> loadPopulations(String ipAddress, int port) {
        List<String> lines = readFileFromSocket(ipAddress, port);
...
class MainRun {
    public static void main(String[] args) {
        ThirdFinal finalInstance = new ThirdFinal("10.14.10.4", 2345);
        CustomFileWriter.writeReport("becir.csv", finalInstance.getPopulations());
    }
}